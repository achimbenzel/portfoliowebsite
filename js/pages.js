...
function pricingPlansHtml(c){
  ...
  return`<section class="svc-plans-band" data-theme="${band}" id="svcPlans"><div class="reveal">`
    +`<div class="svc-plans-head">`
      +`<h3 class="svc-plans-title">${pr.planTitle}</h3>`
...
/* ===== SERVICE CATEGORY PAGE (/branding, /motion-design, /web-design) =====
   Hero -> what you gain -> how it runs -> what you end up with -> packages ->
   real projects -> FAQ -> (branding only) logo shop -> closing CTA.
   No imagery of its own: every section is built from components that already
   exist on the site. */
function svcCatPg(key){
  ...
  /* ---- Hero: eyebrow, headline, lead, two calls to action ---- */
  const heroCta=`<div class="svc-hero-cta">`
    +((c.plans||[]).length
      ?`<a class="svc-jump" href="#svcPlans" onclick="event.preventDefault();jumpToPlans()">${sp.jump} ${downSVG}</a>`:'')
    +`<a class="svc-hero-alt" href="${routeToPath('contact')}" onclick="event.preventDefault();inquire('${deEnt(c.label).replace(/'/g,"\\'")}')">${sp.inquire} ${arrowSVG}</a>`
  +`</div>`;
  const heroHtml=`<div class="reveal svc-hero">`
    +`<span class="svc-eyebrow">${c.label}</span>`
    +`<h2 class="hw svc-hero-title" data-anim="chars" data-anim-stagger="22" data-anim-duration="550">${c.title}</h2>`
    +`<p class="svc-hero-lead" data-anim="words" data-anim-stagger="20">${c.text}</p>`
    +heroCta
  +`</div>`;

  /* ---- What you gain: the three service areas as numbered columns ---- */
  const gainHtml=(c.svcs||[]).length?`<div class="reveal svc-block">`
    +`<h3 class="svc-block-title" data-anim="words" data-anim-stagger="25">${sp.benefits}</h3>`
    +`<div class="svc-gain">${c.svcs.map((v,i)=>`<div class="svc-gain-item"><span class="svc-gain-num">${i+1}.</span><h4 class="svc-gain-t">${v.t}</h4><p class="svc-gain-d">${v.d}</p></div>`).join('')}</div>`
  +`</div>`:'';

  /* ---- Process — the about page's journey timeline, numbered instead of dated ---- */
  const processHtml=(c.process||[]).length?`<div class="reveal svc-block">`
    +`<h3 class="svc-block-title" data-anim="words" data-anim-stagger="25">${sp.process}</h3>`
    +`<div class="abt-journey">${c.process.map((p,i)=>`<div class="abt-journey-step"><div class="abt-journey-marker"><span class="abt-journey-year">0${i+1}</span><span class="abt-journey-dot"></span></div><div class="abt-journey-content"><span class="abt-journey-label">${p.t}</span><p class="abt-journey-text">${p.d}</p></div></div>`).join('')}</div>`
  +`</div>`:'';

  /* ---- What you end up with: card per deliverable, media on top and the text
         underneath, the same way the project cards read. The tile is a tinted
         placeholder until real artwork exists. ---- */
  const deliverHtml=(c.deliver||[]).length?`<div class="reveal svc-block">`
    +`<h3 class="svc-block-title" data-anim="words" data-anim-stagger="25">${sp.deliver}</h3>`
    +`<div class="svcd-grid">${c.deliver.map((d,i)=>`<div class="svcd-card">`
  ...
  /* ---- FAQ — same accordion markup and toggleFaq() as the home page ---- */
  const faqHtmlBlock=(c.faq||[]).length?`<div class="reveal svc-block">`
    +`<h3 class="svc-block-title" data-anim="words" data-anim-stagger="25">${sp.faq}</h3>`
    +`<div class="faq-list">${c.faq.map((f,i)=>`<div class="faq-item" id="faq${i}"><button class="faq-question" onclick="toggleFaq(${i})">${f.q} ${plusSVG}</button><div class="faq-answer"><div class="faq-answer-inner">${f.a}</div></div></div>`).join('')}</div>`
  +`</div>`:'';

  /* ---- Logo shop — branding only, and only items tagged 'logo' ---- */
  ...
  const relHtml=rel.length?`<div class="reveal svc-block">`
    +`<h3 class="svc-block-title" data-anim="words" data-anim-stagger="25">${sp.work}</h3>`
  ...
  const closeHtml=`<div class="reveal svc-block svc-close">`
    +`<h3 class="svc-close-title" data-anim="words" data-anim-stagger="25">${sp.ctaTitle}</h3>`
    +`<div class="svc-close-side">`
      +`<p class="svc-close-text">${sp.ctaText}</p>`
      +`<a class="svc-close-btn" href="${routeToPath('contact')}" onclick="event.preventDefault();inquire('${deEnt(c.label).replace(/'/g,"\\'")}')">${sp.ctaBtn} ${arrowSVG}</a>`
    +`</div>`
  +`</div>`;
  ...
  return`<section class="section svc-cat-page" style="padding-top:9rem">`
    ...
    +heroHtml+gainHtml+processHtml+deliverHtml
  +`</section>`
  ...
  +`<section class="section svc-cat-page">`
    +relHtml+faqHtmlBlock+lgCta+closeHtml
  +`</section>`;
}
