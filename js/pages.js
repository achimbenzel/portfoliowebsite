...
/* ===== SERVICE CATEGORY PAGE (/branding, /motion-design, /web-design) =====
   Intro (what's covered, then straight down to the prices) -> how it runs ->
   what you get -> packages -> FAQ -> related work. No imagery: the sections are
   built from components that already exist elsewhere on the site (the about
   page's journey timeline, the FAQ accordion, the project grid). */
function svcCatPg(key){
  ...
  /* Intro: what this service actually covers, in one short line each, then a
     jump straight down to the packages. Replaces the old .services-grid cards —
     the page now explains briefly and sends the reader to the prices. */
  ...
  const sumHtml=(c.svcs||[]).length
    ?`<ul class="svc-summary">${c.svcs.map((v,i)=>`<li class="svc-summary-item" data-anim="fade" data-anim-delay="${150+i*90}"><span class="svc-summary-num">0${i+1}</span><span class="svc-summary-body"><span class="svc-summary-t">${v.t}</span><span class="svc-summary-d">${v.d}</span></span></li>`).join('')}</ul>`
    :'';
  const introCta=(c.plans||[]).length
    ?`<div class="svc-intro-cta" data-anim="fade" data-anim-delay="480">`
      +`<a class="svc-jump" href="#svcPlans" onclick="event.preventDefault();jumpToPlans()">${sp.jump} ${downSVG}</a>`
      +`<span class="svc-jump-note">${sp.jumpNote.replace('{price}',c.fromPrice||'')}</span>`
    +`</div>`
    :'';
  ...
  const deliverHtml=(c.deliver||[]).length?`<div class="reveal svc-block">`
    ...
    +`<ul class="svc-deliver">${c.deliver.map(d=>`<li class="svc-deliver-item">${tickSVG}<span>${d}</span></li>`).join('')}</ul>`
  +`</div>`:'';
